...
#define SWARM 90 // Number of particles
...
const char *sol_file = "/home/eternal/Documents/pso-algorithm/new_special/solutions/ftv70_sol.txt";
...
{
    getCurrentDateTime(startedAt, sizeof(startedAt));
    srand(1);
    pso();
    return 0;
}
...
void rankPrticles(Particle *particle, Particle *global)
{
    ...
    getCurrentDateTime(updatedAt, sizeof(updatedAt));
    ...
    if (fit < global->bestFit)
    {
        ...
        printf("\r| ITERATION -> %ld | FITNESS -> %ld <- | UPDATED -> %s |", count, fit, updatedAt);
        fflush(stdout); // Flush the output buffer

        // Write Best Found Solution into Disk
        fprintf(solf, "\n\n\t\t\t\t----* %d x %d COST *----\n\n\n", D, D);
        fprintf(solf, "\t\t  STARTED AT : %s TIME\n\n\n", startedAt);
        fprintf(solf, "\t  BEST FITNESS : %5ld | OPTIMUM : %4d | EXECUTING ...\n\n\nBEST PATH (HAVING %ld):\n\n",
                fit, BEST_SOLUTION, fit);

        for (j = 0; j < D; j++)
        {
            global->bestPosition[j] = particle->position[j];
            fprintf(solf, "%3d\t", particle->position[j]);
            if (j % 16 == 0)
            {
                fprintf(solf, "\n");
            }
...
void updatePosition(Particle *Particle)
{
    ...
    simulated_annealing(Particle->position, TEMP, 0.99);
...
void simulated_annealing(int *path, double temp, double cooling_rate)
{
    int tour[D];
    while (temp > 1)
    {

        InverseMutation(path, tour);
...
void rgibnnm(int *tour)
{
    ...
    while (!improved && x < n)
    {
        ...
        int gene = x;
        double old_fit = cal_fitness(tour);
        // Find the nearest neighbor of the selected gene
        int nearest = -1;
        int min_dist = __INT16_MAX__;
        for (int i = 0; i < n; i++)
        {
            if (i != gene)
            {
                int dist = matrix[tour[gene]][tour[i]];
                if (dist < min_dist)
                {
                    min_dist = dist;
                    nearest = i;
        ...
        double new_fit = cal_fitness(tour);
        if (new_fit < old_fit)
            improved = 1;
        else
        {
...
void getCurrentDateTime(char *dateTimeStr, size_t maxSize)
{
    // Obtain current time
    time_t currentTime;
    time(&currentTime);
    ...
    // Determine AM or PM
    char *am_pm = (localTime->tm_hour >= 12) ? "PM" : "AM";
    ...
    snprintf(dateTimeStr, maxSize, "%02d-%02d-%04d %02d:%02d:%02d %s",
             localTime->tm_mday,
             localTime->tm_mon + 1,     // tm_mon is months since January (0-11)

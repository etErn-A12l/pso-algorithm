...
void *updateParticles(void *arg)
{
    ...
    for (i = args->startIndex; i < args->endIndex; i++)
    {
        ...
        updatePosition(&(args->particles[i]), args->globalBest);
...
void updatePosition(Particle *particle, Particle *globalBest)
{
    float max_vel = 0;
    int k;
    // for (k = 0; k < D; k++)
...
void pso()
{
    ...
    do
    {
        ...
        for (i = 0; i < SWARM; i++)
        {
            ...
            // Update particle's position
            updatePosition(&particles[i], &globalBest);
...
void updateUsingMech(Particle *particle, Particle *globalBest)
{
    int newPosition1[D];
    int newPosition2[D];
    new_mech(particle->position, particle->bestPosition, newPosition1, D);
    new_mech(particle->position, globalBest->bestPosition, newPosition2, D);
